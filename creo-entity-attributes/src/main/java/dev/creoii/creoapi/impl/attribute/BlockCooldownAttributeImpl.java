...
public final class BlockCooldownAttributeImpl {
    public static void addAttributes(CallbackInfoReturnable<DefaultAttributeContainer.Builder> cir) {
        cir.getReturnValue().add(RegistryEntry.of(CreoEntityAttributes.PLACE_COOLDOWN)).add(RegistryEntry.of(CreoEntityAttributes.BREAK_COOLDOWN));
    }
    ...
    public static void applyBlockPlaceSpeed(MinecraftClient client, ItemStack stack) {
        if (stack.getItem() instanceof BlockItem && client.player != null) {
            client.itemUseCooldown = (int) client.player.getAttributeValue(RegistryEntry.of(CreoEntityAttributes.PLACE_COOLDOWN));
        }
    }

    public static int applyBlockBreakSpeed(MinecraftClient client) {
        if (client.player != null) {
            return (int) client.player.getAttributeValue(RegistryEntry.of(CreoEntityAttributes.BREAK_COOLDOWN));
        }
        return 5;
    }
}
